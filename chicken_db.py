...
def initialize_db():
    ...
    # Check if LastUpdated column exists (for existing DBs)
    cursor.execute("PRAGMA table_info(Suppliers)")
    columns = [info[1] for info in cursor.fetchall()]
    if 'LastUpdated' not in columns:
        cursor.execute("ALTER TABLE Suppliers ADD COLUMN LastUpdated TEXT")
...
def rename_vendor(old_name, new_name):
    """
    ...
    try:
        # 1. Update Suppliers
        cursor.execute("UPDATE Suppliers SET SupplierName = ?, LastUpdated = ? WHERE SupplierName = ?", 
                       (new_name, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), old_name))
        
        # 2. Update Markups
        cursor.execute("UPDATE Markups SET SupplierName = ? WHERE SupplierName = ?", (new_name, old_name))
        
        # 3. Update BillEntries
        cursor.execute("UPDATE BillEntries SET SupplierName = ? WHERE SupplierName = ?", (new_name, old_name))
        
        # 4. Update VendorLedger
        cursor.execute("UPDATE VendorLedger SET SupplierName = ? WHERE SupplierName = ?", (new_name, old_name))
        
        conn.commit()
        return True
    except Exception as e:
        print(f"Error renaming vendor: {e}")
        conn.rollback()
        return False
    finally:
        conn.close()
...
def fetch_rate_history(limit=30):
    """Fetches the last 'limit' days of rate data for charting."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute(f"""
        SELECT Date, TandoorRate, BoilerRate, EggRate 
        FROM RawData 
        ORDER BY Date DESC 
        LIMIT {limit}
    """)
    ...
    conn.close()
    # Return reversed to show oldest to newest on chart
    return rows[::-1]

def fetch_vendor_dues():
    """Calculates total due for each vendor for charting."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    ...
    for supplier in suppliers:
        # Sum of Bills
        cursor.execute("SELECT IFNULL(SUM(Qty * VendorRate), 0.0) FROM BillEntries WHERE SupplierName = ?", (supplier,))
        bills = cursor.fetchone()[0]
        # Sum of Payments
        cursor.execute("SELECT IFNULL(SUM(Amount), 0.0) FROM VendorLedger WHERE SupplierName = ?", (supplier,))
        payments = cursor.fetchone()[0]
        
        dues[supplier] = round(bills + payments, 2)
