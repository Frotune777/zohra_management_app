...
class VendorManager:
    ...
    def _setup_markup_management_frame(self, frame):
        ...
        # Add Scrollbar
        vsb = ttk.Scrollbar(frame, orient="vertical", command=self.markup_tree.yview)
        self.markup_tree.configure(yscrollcommand=vsb.set)
        
        self.markup_tree.pack(side=tk.LEFT, fill='both', expand=True, pady=10)
        vsb.pack(side=tk.RIGHT, fill='y', pady=10)
    ...
    def _create_markup_combobox_editor(self, item_id, column, col_idx, x, y, w, h, value, options):
        # Use the same font as the Treeview for alignment
        combo = ttk.Combobox(self.markup_tree, values=options, state='readonly', font=('Segoe UI', 9))
        combo.set(value)
        ...
        def save_edit(event):
            ...
            # Unbind to prevent double firing during destroy
    ...
    def _create_markup_entry_editor(self, item_id, column, col_idx, x, y, w, h, value):
        ...
        entry = ttk.Entry(self.markup_tree, font=('Segoe UI', 9))
    ...
    def _save_markup_change(self, item_id, values):
        ...
        try:
            ...
            # Removed annoying success popup
            self.master_app.markup_rules_cache = {} # Clear cache
